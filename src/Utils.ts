...
export default class Utils {
    ...
    * @returns 
    */
   public static prettyPrint(data: any, start?: string, end?: string, prefix: string = ''): string {
      let ret = '';
      if (Array.isArray(data)) {
         ...
         ret = (start || '[') + data.map(i => Utils.prettyPrint(i)).join(', ') + (end || ']');
      } else if (data instanceof Set) {

         ret = (start || '{') + Array.from(data).map(i => Utils.prettyPrint(i)).join(', ') + (end || '}');
      } else {
         ret = data.toString();
      }
