...
export abstract class Token {
   ...
   toString(): string {
      return this._type + ' - "' + this._text + '" (' + this._line + ', ' + this._offset + ')';
   }
}
...
export default abstract class Lexer<T extends Token> {
   ...
   */
   protected lookAhead(): string {
      ...
      if (this.laAbsIndex < this.buffer.length) {
         ...
         this.laLineOffset++;
         if (c == '\n') {
            this.logger.info(`Detected end of line at (${this.laLine}, ${this.laLineOffset})`);
            this.laLine++;
            this.laLineOffset = 0;
         }
         return c;
   ...
   */
   protected fillBufferIfNeeded(): void {
      if (!this.fileEnded && this.buffer.length - this.laAbsIndex < BUFFER_MINLENGTH) {
         this.fillBuffer();

         if (this.cursorAbsIndex > 0) { // Remove unnecessary characters (already used)
   ...
   */
   public isEOF(): boolean {
      const ret: boolean = this.fileEnded && this.cursorAbsIndex == this.buffer.length;
      this.logger.info(`EOF check: ${ret}, File ended: ${this.fileEnded}, cursor Abs Pos: ${this.cursorAbsIndex}, buff length: ${this.buffer.length}`)
   ...
   */
   public getSnapshot(): string {
      return this.buffer.substring(this.cursorAbsIndex, this.buffer.length - this.cursorAbsIndex > 100 ? this.cursorAbsIndex + 100 : this.buffer.length);
   }
   ...
   *
   ...
   public attemptRegex(sre: string, sreAfter: string | null, multiline: boolean = false, cInsensitive: boolean = false): string | null {
      this.fillBufferIfNeeded();
      let localBuff: string = this.buffer.substring(this.cursorAbsIndex, this.buffer.length);
      const gre: RegExp = new RegExp('^(' + sre + ')' + (sreAfter ?? '.*'), (multiline ? 's' : '') + (cInsensitive ? 'i' : '')) // s flag means dot all
      let matches: string[] | null = gre.exec(localBuff);
      let matched: string | null = null;

      if (matches) { // Buffer matches...do with the lookahead to update cursors
         matched = matches[1];
