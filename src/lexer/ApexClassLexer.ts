...
const TOKENTYPE_CODE_MULTIPLYASSIGNMENT = new TokenType(
  "CODE_MULTIPLYASSIGNMENT"
);
...
export {
  ...
  TOKENTYPE_CODE_STRINGLITERAL,
  TOKENTYPE_CODE_QUERY,
  ...
  TOKENTYPE_CODE_LESSEQUAL,
  TOKENTYPE_CODE_LESS,
  TOKENTYPE_CODE_EQUALS,
  TOKENTYPE_CODE_GREATEREQUAL,
  TOKENTYPE_CODE_GREATER,
  TOKENTYPE_CODE_NOTEQUALS,
  TOKENTYPE_CODE_ASSIGN,
  TOKENTYPE_CODE_DOT,
  TOKENTYPE_CODE_ADD,
  TOKENTYPE_CODE_SUB,
  TOKENTYPE_CODE_MULTIPLY,
  TOKENTYPE_CODE_DIVIDE,
  TOKENTYPE_CODE_SUBUNARY,
  TOKENTYPE_CODE_ADDUNARY,
  TOKENTYPE_CODE_NUMBER,
...
export default class ApexClassLexer extends Lexer<ApexToken> {
  ...
  nextTokenOnCode(): ApexToken | null {
    ...
    if ((tokenText = this.attemptRegex("/\\*{2,}", null)) != null) {
      ...
      (tokenText = this.attemptRegex("'[^'\\\\]*(?:\\\\.[^'\\\\]*)*'", null)) !=
      null
    ) {
      token = new ApexToken(
        TOKENTYPE_CODE_STRINGLITERAL,
      ...
      (tokenText = this.attemptRegex("[0-9]+(?:\\.[0-9]+)?", null)) != null
    ) {
      token = new ApexToken(
        TOKENTYPE_CODE_NUMBER,
        tokenText,
      ...
      token = new ApexToken(
        TOKENTYPE_CODE_MULTIPLYASSIGNMENT,
        tokenText,
        this.cursorLine,
        this.cursorLineOffset
      );
      ...
      token = new ApexToken(
        TOKENTYPE_CODE_DIVIDEASSIGNMENT,
        tokenText,
        this.cursorLine,
        this.cursorLineOffset
      );
  ...
  nextTokenOnComment(): ApexToken | null {
    ...
    if ((tokenText = this.attemptRegex("[^ \\n\\t]*\\*+/", null)) != null) {
      ...
      (tokenText = this.attemptRegex("@[a-zA-Z0-9]+", BLANKS)) != null
    ) {
      token = new ApexToken(
        TOKENTYPE_COMMENT_JD_ANNOTATION,
        tokenText,
  ...
  nextToken(): Token {
    ...
    if (!token) {
      throw new Error(
        "Found problem reading file (" +
          this.isEOF() +
          "). No tokens identified: " +
          this.getSnapshot()
      );
    }
    return token;
