...
export abstract class Parser<T extends Token, Z extends IR> {
   ...
   */
   protected rollbackAttempt(): void {
      if (this.tokenIdxStack.length == 0) {
         throw new Error(
            `Cannot rollback when not attempt is in place...${this.tokenIdxStack}`
         );
      }
   ...
   */
   protected log(message: string) {
      ...
      const text:string = `${depthString} ${
            this.pushedContexts.length > 0
            ? "[" + this.pushedContexts[this.pushedContexts.length - 1] + "]"
            : "[--]"
         } - ${message}`;
   ...
   */
   protected pushContext(context: ParseContext): void {
      ...
      this.log(
         `[P][${
            attempting ? "a" : "na"
         }] >>>> Context stack is ${Utils.prettyPrint(
            this.pushedContexts
   ...
   */
   protected nextToken(): T {
      ...
      if (ret) {
         // If there are still tokens continue reading for next request
         this.tokenIndex++;
         if (this.tokenIndex == this.tokens.length) {
            // Read new token for the lookahead
            const token: T = this.lexer.nextToken();
            this.tokens.push(token);
         }
      }
   ...
   protected createSnapshot(): string {
      let ret: string = "";
      let line: number = 0;
      let maxTokens:number = 20; // Not more than 20 tokens
      ...
      return `///${this.tokens[this.tokenIndex].line}:${
         this.tokens[this.tokenIndex].offset
      } - ${ret} - ${this.tokens[this.tokens.length - 1].line}:${
         this.tokens[this.tokens.length - 1].offset
      }///`;
   }
