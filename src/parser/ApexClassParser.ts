...
import ApexClassLexer, { ApexToken, TOKENTYPE_CODE_ANNOTATION, TOKENTYPE_CODE_ASSIGN, TOKENTYPE_CODE_CBRACKET_END, TOKENTYPE_CODE_CBRACKET_START, TOKENTYPE_CODE_COMMA, TOKENTYPE_CODE_GREATER, TOKENTYPE_CODE_LESS, TOKENTYPE_CODE_RBRACKET_END, TOKENTYPE_CODE_RBRACKET_START, TOKENTYPE_CODE_RESERVEDWORD, TOKENTYPE_CODE_SBRACKET_END, TOKENTYPE_CODE_SBRACKET_START, TOKENTYPE_CODE_SEMICOLON, TOKENTYPE_CODE_WORD, TOKENTYPE_COMMENT_JD_ANNOTATION, TOKENTYPE_COMMENT_JD_START, TOKENTYPE_COMMENT_ML_END, TOKENTYPE_COMMENT_ML_START, TOKENTYPE_COMMENT_SL, TOKENTYPE_COMMENT_WORD } from "../lexer/ApexClassLexer.js";
...
import { JD_ANNOTATIONS_WITHPARAMS, CLASS_ACCESSMODIFIERS, WORD_ABSTRACT, WORD_CLASS, WORD_EXTENDS, WORD_IMPLEMENTS, WORD_INTERFACE, WORD_VIRTUAL, WORD_WITH, WORD_WITHOUT, WORD_STATIC, WORD_TESTMETHOD, WORD_OVERRIDE, APX_ANNOTATION_ISTEST, WORD_FINAL, WORD_ENUM } from "../ir/direct/apex/ApexConstants.js";
...
export default class ApexClassParser extends Parser<ApexToken, ApexIR> {
   ...
   private doJDCommentAnnotation(): JDCommentAnnotationIR {
      ...
      try {
         ...
         if (JD_ANNOTATIONS_WITHPARAMS.has(ret.name) && token.type === TOKENTYPE_COMMENT_WORD) {
         ...
         while ((token = this.lookAhead()).line === line
         ...
         this.log(`Created jd comment annotation ${ret} - Next token ${this.lookAhead()}`);
         return ret;
      } finally {
         this.popContext();
      }
   ...
   private doType(): ApexTypeIR {
      ...
      try {
         ...
         let ret: ApexTypeIR = new ApexTypeIR(token.text);
         this.nextToken();
         token = this.lookAhead();
         ...
         while (token.type === TOKENTYPE_CODE_SBRACKET_START) { // Array definition
            this.nextToken();
            token = this.lookAhead();
            if (token.type !== TOKENTYPE_CODE_SBRACKET_END) {
               this.log(`Found the start square bracket but not the close one after ${token}`);
               throw this.failedAttemptError;
            }
            ret.arrayDimensions++;
            this.nextToken();
   ...
   private doApexAnnotation(): ApexAnnotationIR {
      ...
      this.nextToken();
      token = this.lookAhead();
      if (token.type === TOKENTYPE_CODE_RBRACKET_START) { // Has parameters
         this.nextToken();
         token = this.lookAhead();
         while (token.type !== TOKENTYPE_CODE_RBRACKET_END) {
            ...
            if (token.type === TOKENTYPE_CODE_ASSIGN) { // Has value
               this.nextToken();
               token = this.lookAhead();
               value = token.text;
            }
            ...
            this.nextToken();
            token = this.lookAhead();
            if (token.type === TOKENTYPE_CODE_COMMA) { // More params
               this.nextToken();
               token = this.lookAhead();
            } else if (token.type !== TOKENTYPE_CODE_RBRACKET_END) {
               throw new ParserError(`Unknown structure found in annotation. Expected comma "," or bracket close ")" but found ${token}`, token.line, token.offset);
            }
         }
         this.nextToken();
      }
   ...
   private doParameter(): ApexParameterIR {
      ...
      try {
         ...
         if (token.type !== TOKENTYPE_CODE_WORD) {
            this.log(`Problem reading parameter. Expected name of parameter but found ${token}`);
            throw this.failedAttemptError;
         }
   ...
   private doMethodBody(method: ApexMethodIR): void {
      ...
      try {
         ...
         if (token.type !== TOKENTYPE_CODE_CBRACKET_START) {
            this.log(`Expected method body to start with curly bracket "{" but found ${token}`);
            throw this.failedAttemptError;
         }
         this.nextToken();
         token = this.lookAhead();
         while (true) {
            if (this.attempt('mlComment', this.doMLComment)) {
               ...
               nesting--;
               this.nextToken();
               if (nesting < 0) { // We start with nesting 0 and the end one is the closing one that was not counted in the nesting level 
                  break;
               }
   ...
   private doMethod(isClass: boolean): ApexMethodIR {
      ...
      try {
         ...
         if (token.type !== TOKENTYPE_CODE_RBRACKET_START) {
            this.log(`Expected open bracket for method parameters but found ${token}`);
            throw this.failedAttemptError;
         }
         ...
         while (token.type !== TOKENTYPE_CODE_RBRACKET_END) {
            ...
            if (token.type === TOKENTYPE_CODE_COMMA) {
               ...
               this.log(`Expected closing bracket ")" or "," for method definition but found "${token}"`);
               throw this.failedAttemptError;
            }
            guard++;
            if (guard > 100) {
         ...
         if (isAbstract || !isClass) {
            if (token.type !== TOKENTYPE_CODE_SEMICOLON) {
               this.log(`Expected semicolon ";" on ${isAbstract ? 'abstract' : 'interface'} method but found ${token}`)
            }
            this.nextToken();
            token = this.lookAhead();
         } else {
            if (token.type !== TOKENTYPE_CODE_CBRACKET_START) {
               this.log(`Expected curly bracket "{" as the method block start but found ${token}`);
               throw new ParserError(`Expected curly bracket "{" as the method block start but found ${token}`, token.line, token.offset, CODE_MALFORMED);
            }
   ...
   private doConstructor(className: string): ApexMethodIR {
      ...
      try {
         ...
         if (token.type !== TOKENTYPE_CODE_WORD) {
            this.log(`Expected identifier as the name of the constructor but found ${token}`);
            throw this.failedAttemptError;
         }
         if (token.ltext !== className.toLowerCase()) {
            this.log(`Expected constructor name to match ${className} but found ${token}`);
            throw this.failedAttemptError;
         }
         ...
         if (token.type !== TOKENTYPE_CODE_RBRACKET_START) {
            this.log(`Expected open bracket "(" after constructor but found ${token}`);
            throw this.failedAttemptError;
         }
         this.nextToken();
         token = this.lookAhead();
         while (token.type !== TOKENTYPE_CODE_RBRACKET_END) {
            ...
            if (token.type === TOKENTYPE_CODE_COMMA) {
               ...
               this.log(`Expected ")" or "," but found ${token} in constructor parameters`);
               throw this.failedAttemptError;
            }
         }
         this.nextToken();
         ...
         if (token.type !== TOKENTYPE_CODE_CBRACKET_START) {
            throw new ParserError(`Expected opening curly brace after constructor but found ${token}`, token.line, token.offset, CODE_MALFORMED);
         }
   ...
   private doStaticBlock(): ApexStaticBlockIR {
      ...
      try {
         ...
         let token: Token = this.lookAhead();
         if (token.type !== TOKENTYPE_CODE_RESERVEDWORD || token.ltext !== WORD_STATIC) {
            throw this.failedAttemptError;
         }
   ...
   private ignoreBlock(): void {
      ...
      if (token.type !== TOKENTYPE_CODE_CBRACKET_START) {
         throw new ParserError(`Cannot ignore block that does not start with "{" -> ${token}`, token.line, token.offset, CODE_MALFORMED);
      }
      let nesting: number = 0;
      do {
         this.nextToken();
         ...
         if (token.type === TOKENTYPE_CODE_CBRACKET_START) {
            ...
            throw new ParserError(`Unable to ignore block that started on ${startToken}. Reached EOF`, startToken.line, startToken.offset, CODE_MALFORMED);
         }
      } while (true);
      this.nextToken();
   }
   ...
   private ignoreComments(): void {
      while (true) {
         let token: ApexToken = this.lookAhead();
         if (token.type == TOKENTYPE_COMMENT_ML_START || token.type == TOKENTYPE_COMMENT_JD_START) {
            do {
   ...
   private doEnum(): ApexEnumIR {
      ...
      try {
         ...
         if (token.type !== TOKENTYPE_CODE_WORD) {
            ...
            throw new ParserError(`Expected identifier after reserved word enum but found ${token}`, token.line, token.offset, CODE_MALFORMED);
         }
         ...
         token = this.lookAhead();
         if (token.type !== TOKENTYPE_CODE_CBRACKET_START) {
            throw new ParserError(`Expected curly bracket after enum name "{" but found ${token}`, token.line, token.offset, CODE_MALFORMED);
         }
         this.nextToken();
         token = this.lookAhead();
         ...
         while (token.type !== TOKENTYPE_CODE_CBRACKET_END) {
            ...
            if (token.type === TOKENTYPE_CODE_WORD) {
               ...
               throw new ParserError(`No identifier found for enum at token ${token}`, token.line, token.offset, CODE_MALFORMED);
            }
            this.ignoreComments();
            token = this.lookAhead();
            if (token.type === TOKENTYPE_CODE_COMMA) {
               ...
               throw new ParserError(`Expected a comma "," or closing curly bracket "}" after enum value but found ${token}`, token.line, token.offset, CODE_MALFORMED);
            }
   ...
   private doAttribute(): ApexAttributeIR {
      ...
      try {
         ...
         token = this.lookAhead();
         if (token.type !== TOKENTYPE_CODE_WORD) {
            this.log(`Expected attribute identifier to be of type word but found ${token}`);
            throw this.failedAttemptError;
         }
   ...
   private doClassBody(classIR: ApexClassIR): void {
      ...
      try {
         let token: Token = this.lookAhead();
         if (token.type !== TOKENTYPE_CODE_CBRACKET_START) {
            throw new ParserError(`Expected char "{" after class declaration but found ${token}`, token.line, token.offset, CODE_MALFORMED);
         }

         this.nextToken();
         token = this.lookAhead();
         while (token.type !== TOKENTYPE_CODE_CBRACKET_END && token.type !== TOKENTYPE_EOF) {
            ...
            if (this.attempt('method', () => this.doMethod(classIR.isClass))) {
            ...
            } else if (this.attempt('constructor', () => this.doConstructor(classIR.name))) {
               ...
               throw new ParserError(`Unable to understand structure in class body after token ${token}`, token.line, token.offset, CODE_MALFORMED);
            }
         ...
         if (token.type !== TOKENTYPE_CODE_CBRACKET_END) {
            throw new ParserError(`No leading "}" found`, token.line, token.offset, CODE_MALFORMED);
         }
   ...
   private doClass(): ApexClassIR {
      ...
      try {
         ...
         while (detectedModifier) {
            if (token.text.toLowerCase() === WORD_WITH || token.text.toLowerCase() === WORD_WITHOUT) {
         ...
         if (token.text.toLowerCase() === WORD_CLASS) {
            ...
            this.log(`No class nor interface detected on token ${token}. Desisting...`);
            throw this.failedAttemptError;
         }
   ...
   public parse(): ApexIR {
      ...
      try {
         if (this.attempt('class', this.doClass)) {
            ...
            throw new ParserError(`Unable to identify structure ${this.lookAhead()}`, this.lookAhead().line, this.lookAhead().offset, CODE_MALFORMED, this.createSnapshot());
         }
